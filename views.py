...
def home_view(win_prop, frame=''):
    ...
    HEADING = "Welcome to JJMENSAH Bank"
    ...
    ## Button creation: using data and design variables
    # Admin Button
    Button(frame, 
      text=options[0], 
      font=f"{DEFAULT_FONT} {DEFAULT_FONT_SIZE} bold", 
      bd=0, 
      bg=THEME_DARK, 
      fg=THEME_LIGHT, 
      width=BTN_WIDTH, 
...
def admin_login(win_prop,preframe):
    ...
    Label(frame,text = HEADING,
          font=f'{DEFAULT_FONT} 18 bold', 
          bg = THEME_LIGHT, 
          fg = THEME_DARK).place(x = place_center(WIN_WIDTH,len(HEADING) * 12),
                                 y = WIN_HEIGHT * 0.2)
    # Password label
    ...
    Label(frame, text = PASS_LABEL, 
          ...
          bg=THEME_LIGHT, 
          fg=THEME_DARK).place(x = place_center(WIN_WIDTH,len(PASS_LABEL) * 9),
                               y = WIN_HEIGHT * 0.3)
    ...
    Entry(frame, show='*', 
         ...
         fg='#666', 
         width=BTN_WIDTH, 
         bd=1).place(x = place_center(WIN_WIDTH,BTN_WIDTH * 6.5),
                     y = WIN_HEIGHT * 0.4)
    ...
    Button(frame, text="Login",
           ...
           ).place(x = place_center(WIN_WIDTH, BTN_WIDTH * 7.5), 
                   y = WIN_HEIGHT * 0.5)
    # Back button
    ...
    Button(frame, text="<<👈 Go Back",
           ...
           ).place(x = place_center(WIN_WIDTH, BTN_WIDTH * 7.8), 
                   y = WIN_HEIGHT * 0.6)
